...
echo $Msg
ssh -i sshkey -o "StrictHostKeyChecking no" $USER@$IP 'sudo service lora-tb-conn stop && /home/$USR/sources/deploy-lora-tb-conn.sh && echo "VER='${RELEASE}'" > /home/dev/lora-tb-connector-env && sudo service lora-tb-conn start '
echo $?
if [ $? -eq 0 ]
then
